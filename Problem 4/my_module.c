...
*/
	fl = kzalloc(MEM_SIZE, GFP_KERNEL);
	printk(KERN_INFO"Initialized memory space.\n");
	return 0;
}
...
*/
	kfree(fl);
	printk(KERN_INFO"Released memory space.\n");
	return 0;
}
...
*/
	if(access_ok(buf, len) > 0){
		printk(KERN_INFO"User space pointer is readable.\n");
		if(copy_to_user(buf, fl, len) == 0){
...
*/
	if(access_ok(buf, len) > 0){
		printk(KERN_INFO"User space pointer is writable.\n");
		if(copy_from_user(fl, buf, len) == 0){
